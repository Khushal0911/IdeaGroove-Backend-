...
export const getUserGroups = async (req, res) => {
    try {
        ...
        res.status(500).json({ status: false, error: "Failed to fetch your groups" });
    }
};
...
export const addGroup = async(req,res)=>{
    ...
    try{
        ...
        if(addResult.affectedRows > 0){
            ...
            const [adminResult] = await connection.query(addAdminQuery, [newRoomId, Created_By]);
...
export const deleteGroup = async(req,res)=>{
    ...
    try{
        ...
        const [deleteResult] = await connection.query(deleteGroupQuery,[
            Room_ID
        ]);
...
export const joinGroup = async(req,res)=>{
    ...
    try{
        ...
        const [existing] = await connection.query(`SELECT * FROM chat_room_members_tbl
...
export const leaveGroup = async(req,res)=>{
    ...
    try{
        ...
        if (!memberInfo.length || memberInfo[0].Is_Active === 0) {
            await connection.rollback();
            return res.status(400).json({ status: false, message: "User is not an active member of this group" });
        }
      ...
      //mark the current user as left
        ...
        if (leaveResult.affectedRows > 0) {
            ...
            res.status(200).json({ 
                status: true, 
                message: userRole === 'admin' ? "Left group and promoted new admin" : "Left group successfully" 
            });
        } else {
            await connection.rollback();
